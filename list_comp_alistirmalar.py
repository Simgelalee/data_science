...
# Sütun isimlerini değiştirme işlemi
df.columns = ["NUM_" + col.upper() if df[col].dtype == "float64" else col.upper() for col in df.columns]

# Yeni sütun isimlerini yazdır
print(df.columns)
df.head()
...
df.columns = [col if "NO" in col else col.upper() + "FLAG" for col in df.columns]
print(df.columns)
